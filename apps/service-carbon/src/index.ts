/**
 * Carbon service entry point.
 *
 * A standalone x402 resource server. It knows nothing about AtomicAgent, its
 * atomic group, or which slot it might be paid from — it verifies whatever
 * payment index a client declares and serves its estimate.
 *
...
app.use(
  cors({
    // Deliberately open. A service published for anyone to discover and pay
    // cannot know which origins will call it, and restricting them would
    // undermine the point of being independently discoverable.
    origin: true,
    methods: ['GET', 'POST'],
 ...
 * reaches the generic handler and surfaces as a 500 — which wrongly implies our
 * service is broken when the client simply sent bad data. A client error
 * deserves a 4xx.
 *
...
app.get('/health', (_req, res) => {
  res.status(200).json({
    ok: true,
    data: {
      service: 'service-carbon',
      status: 'healthy',
      network: env.network,
      payTo: env.payTo,
      asset: env.asset,
      feeAtomic: env.feeAtomic,
      // Deliberately absent: this service has no fixed slot. It verifies
      // whichever payment index a client declares, which is what lets it be
      // paid from any position in any group.
      paymentIndex: null,
      slotAgnostic: true,
    },
  });
...
const server = app.listen(env.port, '0.0.0.0', () => {
  logger.info(
    ...
    `carbon service listening on http://localhost:${env.port}`,
  );
});
