 ...
 * responses. Internal details are logged but never sent to the client.
 */

import type { ErrorRequestHandler, RequestHandler } from 'express';
import { ZodError } from 'zod';
import { ERROR_CODE } from '@atomicagent/shared';
import { isAppError } from '@atomicagent/shared';
 ...
 * recognise it as an error handler, which is why `next` is present but unused.
 */
export const errorHandler: ErrorRequestHandler = (err, req, res, _next) => {
  ...
  if (isAppError(err)) {
    ...
    logger[level](
      { path: req.path, code: err.code, detail: err.detail, runId: err.runId },
      err.message,
    );
 ...
 * inside an async route hangs the request until the client times out.
 *
