 ...
 *
 * Every service validates its config at startup with Zod and exits immediately
 * if anything is missing or malformed. A service that boots with a bad address
 * and fails on the first request during a live demo is far worse than one that
...
const envSchema = z.object({
  ...
  SVC_PRICE_ADDRESS: z
    .string()
    .regex(ALGORAND_ADDRESS_REGEX, 'SVC_PRICE_ADDRESS must be a 58-char Algorand address'),
  ...
  FEE_PRICE: z.string().regex(/^\d+$/, 'FEE_PRICE must be atomic units (digits only)'),
...
if (!parsed.success) {
  ...
  console.error('\n[service-price] Invalid environment configuration:\n');
